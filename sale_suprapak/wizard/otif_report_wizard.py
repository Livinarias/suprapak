...
class OtifReportWizard(models.TransientModel):
    ...
    def generate_file(self):
        ...
        for i, order in enumerate(orders):
            ...
            if self.start_datetime <= order.expected_date or order.expected_date >= self.end_datetime:
                pass
            ...
            worksheet.write(i + 5, 8, order.sheet_id.reference or '',general)
            worksheet.write(i + 5, 9, order.sheet_id.presentation_id.name or '',general)
            #worksheet.write(i + 5, 10, order.sheet_id.material_id.name or '',general) tipo pedido
            worksheet.write(i + 5, 11, order.sheet_id.presentation_id.name or '',general)
            worksheet.write(i + 5, 12, order.sheet_id.reference or '',general)
